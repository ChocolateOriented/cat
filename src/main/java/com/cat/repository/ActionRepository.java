...
public interface ActionRepository extends JpaRepository<Action, Long> {
	...
	@Query(value = "select a.id, a.customer_id, a.order_id, t.order_status, t.repayment_time,"
			...
			+ " from t_cat_action a join t_cat_task t on a.order_id = t.order_id where t.customer_id = ?1"
			+ " order by a.create_time desc /*#pageable*/",
			countQuery = "select count(*) from t_cat_action a where a.customer_id = ?1 /*#pageable*/",
			nativeQuery = true)
